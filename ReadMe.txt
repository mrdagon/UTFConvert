...
　何らかの理由でclangで無くGCCが使いたい場合にも利用出来ると思います。

●使い方
　変換したいソースやヘッダーファイルを変換UTF.exeにドロップすると、文字が変換されます、変換後のソースをndk-buildして下さい。
...
　文字リテラルを変換しない処理の関係でコメント中に"が一つ存在している場合、変換が正しく行われない事があります。
...
配布しているTD等は仮引数以外にも日本語の識別子を使っているため、ソースの変換が必要になります。
